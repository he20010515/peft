...
from peft import PrototypeLoraModel, PrototypeLoraConfig, PeftModel, GlobalMemLoraModel, GlobalMemLoraConfig
import json
...
peft_config = GlobalMemLoraConfig(
    task_type=TaskType.CAUSAL_LM, inference_mode=False, r=4, lora_alpha=32, lora_dropout=0.1, codebook_input_channel=8
)
model = AutoModel.from_pretrained(model_name_or_path, cache_dir="/data/qibiqing/pub_models_bqqi")
model = get_peft_model(model, peft_config)
model.print_trainable_parameters()
p = model.all_parameters()
...
# output: trainable params: 2359296 || all params: 1231940608 || trainable%: 0.19151053100118282
with open("global.p.json", "w") as f:
    json.dump(p, f)
